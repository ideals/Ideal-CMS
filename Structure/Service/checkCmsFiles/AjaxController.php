...
class AjaxController extends \Ideal\Core\AjaxController
{
    ...
    public function checkCmsFilesAction()
    {
        ...
        $actualSystemFilesHash = self::getAllSystemFiles(DOCUMENT_ROOT . '/' . $config->cmsFolder . '/Ideal', DOCUMENT_ROOT . '/' . $config->cmsFolder);

        // Подгружаем имеющуюся информацию о хэшах системных файлов
        $existingSystemFilesHash = require_once(DOCUMENT_ROOT . '/' . $config->cmsFolder . '/Ideal/setup/prepare/hash_files.php');
