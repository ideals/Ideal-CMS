...
class GsgXml
{
    ...
    function addUrl(
        ...
        if ($pathOnly) {
            ...
            $url = substr($url, 0, $this->baseUrlLen - 1) . preg_replace(
                    '|/+|',
                    '/',
                    substr($url, $this->baseUrlLen - 1)
                ); // replace double slashes with a single slash
        } else {
            if ($this->baseUrlLen > 0) {
...
xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd">'
        ...
        $ret[] = sprintf(
            ...
            date($this->lastModDateTime) . substr(date("O"), 0, 3) . ":" . substr(date("O"), 3)
        );
