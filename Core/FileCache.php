...
class FileCache
{
    ...
    public static function saveCache($content, $uri)
    {
        ...
        $excludeCacheFileValue = FileCache::getConfigArrayFile(DOCUMENT_ROOT . '/tmp/cache/exclude_cache.php');
        ...
        if (array_search($uri, $excludeCacheFileValue) === false) {
            ...
            FileCache::checkDir($cacheDir);

            $cacheFile = $cacheDir . '/site_cache.php';
            $cacheFileValue = FileCache::getConfigArrayFile($cacheFile);
     ...
     */
    public static function getConfigArrayFile($path)
    {
        if (file_exists($path)) {
