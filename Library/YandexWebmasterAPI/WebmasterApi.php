...
class WebmasterApi
{
     ...
     * В случае если передан массив $data - его значения будут записаны в запрос. Подробнее об этом массиве см. в описании
     * метода dataToString
     *
     *
    ...
    protected function get($resource, $data = array())
    {
        ...
        if (!$response) {
            return $this->errorCritical('Error in curl when get [' . $url . '] ' . $curl_error);
        }
    ...
    private function dataToString($data)
    {
        ...
        foreach ($data as $param => $value) {
            if (is_string($value) || is_int($value) || is_float($value)) {
                $new_data[] = urlencode($param) . "=" . urlencode($value);
    ...
    {
        if (!$date_from) {
            $date_from = time() - 1209600;
        }
        if (!$date_to) {
            $date_to = time();
        }
        if (!intval($date_to) || !$date_to) {
            return $this->errorCritical("Bad timestamp to \$date_to");
        }
        if (!intval($date_from) || !$date_from) {
            return $this->errorCritical("Bad timestamp to \$date_from");
        }
     ...
     * @return object
     */
    public static function getAccessToken($code, $client_id, $client_secret)
    {
