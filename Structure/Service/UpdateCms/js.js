...
function Update(moduleName, version, url, modalBox) {
    ...
    this.dataCheck = function(data) {
        ...
        if (data.error == 'undefined' && result != true ) {
            this.print('Сбой в работе при выполнении действия' + action + '<br />' + result, 'error');
            return false;
        }
        ...
        if ((data.message == null) && (result.error == true)) {
    ...
    this.print = function(data, type) {
        ...
        switch (type) {
            ...
            case ('message'): classBlock = 'alert alert-info fade in'; break;
